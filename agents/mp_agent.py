...
def open_twin_in_row(board, row, player):
    ...
    return (
        count_player_row(board, player, row) == 2
        and count_player_row(board, ".", row) == 1
    )
...
def open_twin_in_col(board, col, player):
    ...
    return (
        count_player_column(board, player, col) == 2
        and count_player_column(board, ".", col) == 1
    )
...
def open_twin_in_diag1(board, player):
    ...
    return (
        count_player_diag1(board, player) == 2 and count_player_diag1(board, ".") == 1
    )
...
def open_twin_in_diag2(board, player):
    ...
    return (
        count_player_diag2(board, player) == 2 and count_player_diag2(board, ".") == 1
    )
...
def open_single_in_row(board, row, player):
    """ is there an open twin for the player indicated in the row? """
    return (
        count_player_row(board, player, row) == 1
        and count_player_row(board, ".", row) == 2
    )
...
def open_single_in_col(board, col, player):
    """ is there an open twin for the player indicated in the column? """
    return (
        count_player_column(board, player, col) == 1
        and count_player_column(board, ".", col) == 2
    )
...
def open_single_in_diag1(board, player):
    """ is there an open twin for the player indicated in the right diagonal? """
    return (
        count_player_diag1(board, player) == 1 and count_player_diag1(board, ".") == 2
    )
...
def open_single_in_diag2(board, player):
    """ is there an open twin for the player indicated in the left diagonal? """
    return (
        count_player_diag2(board, player) == 1 and count_player_diag2(board, ".") == 2
    )


def killer_corner_exists(board, player, row, col):
    """ is there an empty corner at the row+column indicated for the player
    which has no opponent pieces in the row and colunm, and has at least 1
    player piece in two of the intersecting rows, columns or diagonals? """
    ...
    return (board[row][col] == ".") and (
        ...
        or (
            ...
            and (
                count_player_diag(board, player, row, col) == 1
                and count_player_diag(board, ".", row, col) == 2
            )
        )
        or (
            (
                count_player_column(board, player, col) == 1
                and count_player_column(board, ".", row) == 2
            )
            and (
                count_player_diag(board, player, row, col) == 1
                and count_player_diag(board, ".", row, col) == 2
...
def killer_side_exists(board, player, row, col):
    """ is there an empty mid side position at the row+column indicated for the
    player which has no opponent pieces in the row and colunm, and has at least
    1 player piece in each of the rows and columns? """
    # corners - (row + column), or (row + diag) or (column + diag)
    return (board[row][col] == ".") and (
        (
            (
...
def complete_twin(board, player1, player2):
    ...
    for i in range(len(board[0])):
        if open_twin_in_row(board, i, player1):
            # Got the open twin
            for j in range(len(board[1])):
                if board[i][j] == ".":
...
def make_a_twin(board, player):
    """An open single exists in some row, column or diagonal
       Make it into a twin - this may help improve win score against very naive
       agents. If we were playing a human agent - I would leave a space.
...
def killer_move_exists(board, player):
    ...
    return (
        killer_corner_exists(board, player, 0, 0)
        or killer_corner_exists(board, player, 0, 2)
        or killer_corner_exists(board, player, 2, 0)
        or killer_corner_exists(board, player, 2, 2)
        or killer_side_exists(board, player, 0, 1)
        or killer_side_exists(board, player, 1, 0)
        or killer_side_exists(board, player, 1, 2)
        or killer_side_exists(board, player, 2, 1)
    )
...
def move_number(board, player):
    """Poorly named - this function returns the number of pieces on the board
...
def count_player_row(board, player, row):
    """Again poorly and inconsistently named - - this function returns the
...
def count_player_diag(board, player, row, col):
    """Again poorly and inconsistently named - - this function returns the
    number of pieces on the board in the right diagonal (if row == column),
    else the left diagonal, for the given player"""
    if row == col:
...
def count_player_diag1(board, player):
    """Again poorly and inconsistently named - - this function returns the
    number of pieces on the board in the right diagonal for the given player"""
    # return the number of player counters on the board
    count_p = 0
    row = 0
    col = 0
...
def count_player_diag2(board, player):
    """Again poorly and inconsistently named - - this function returns the
    number of pieces on the board in the left diagonal for the given player"""
    # return the number of player counters on the board
    count_p = 0
    row = 0
    col = len(board[1]) - 1
...
def agent_mp(board_string: str) -> str:
    """Matthew's final agent - an evolved strategy based on the group strategy
    with a few tweaks
    """
    # Convert string to 2D construct
    board = string_to_grid(board_string, 3, 3)
    # are we finished?
    ...
    elif open_single_exists(board, "X"):  # late mod - try to make a twin """
        board = make_a_twin(board, "X")
    else:
        board = make_random_move(board)
        # print("random")

    new_board = grid_to_string(board)
    return new_board


def agent_g1(board_string: str) -> str:
    """The group strategy - Matthew, Brenda, Hrishi
    As I have interpreted it into code
    Hopefully the language is self-explanatory
    """
    # Convert string to 2D construct
    board = string_to_grid(board_string, 3, 3)
    # are we finished?
    ...
    if empty_board(board):
        # print("empty")
        board[0][0] = "X"  # top left as good as any - could randomise
        ...
        board[1][1] = "X"
        # print("go centre 2")
    # REMOVED - NOT PART OF THE ORIGINAL STRATEGY
    #    elif (killer_move_exists(board, 'X')):
    #        board = make_killer_move(board, 'X','X')
    # print("killer for the win")
    # END OF REMOVE
...
def agent_c1(board_string: str) -> str:
    """The group strategy - Matthew, Brenda, Hrishi - tweaked for Centre First
    instead of corner first. And added a more aggressive "try to win" move
    And the late tweak of trying to make 2 in a row.
    """
    # Convert string to 2D construct
    board = string_to_grid(board_string, 3, 3)
    # are we finished?
